...
func TestResolveLogFilePath_WithDataDir(t *testing.T) {
	t.Setenv("DATA_DIR", "/tmp/fast-frame-data")
	got := resolveLogFilePath("")
	want := filepath.Join("/tmp/fast-frame-data", "logs", "fast-frame.log")
	if got != want {
...
func TestBuildFileCore_InvalidPathFallback(t *testing.T) {
	...
	opts.Output.FilePath = filepath.Join(os.DevNull, "logs", "fast-frame.log")
