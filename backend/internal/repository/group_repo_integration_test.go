...
func (s *GroupRepoSuite) TestListWithFilters_IsExclusiveFilter() {
	g1 := &service.Group{
		Name:             "g1",
		RateMultiplier:   1.0,
		IsExclusive:      false,
	...
	isExclusive := true
	groups, page, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, service.StatusActive, "", &isExclusive)
