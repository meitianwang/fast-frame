...
const (
	...
	githubRepo     = "meitianwang/fast-frame"
...
func (s *UpdateService) PerformUpdate(ctx context.Context) error {
	...
	tempDir, err := os.MkdirTemp(exeDir, ".fast-frame-update-*")
	if err != nil {
	...
	newBinaryPath := filepath.Join(tempDir, "fast-frame")
...
func (s *UpdateService) extractBinary(archivePath, destPath string) error {
	...
	if strings.Contains(archivePath, ".tar") {
		...
		for {
			...
			if baseName == "fast-frame" || baseName == "fast-frame.exe" {
