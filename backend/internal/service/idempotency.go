...
import (
	...
	"github.com/meitianwang/fast-frame/internal/util/logredact"
)
...
func RetryAfterSecondsFromError(err error) int {
	...
	raw, ok := appErr.Metadata["retry_after"]
	if !ok {
		return 0
	}
	sec, parseErr := strconv.Atoi(raw)
...
func (c *IdempotencyCoordinator) Execute(
	...
	if execErr != nil {
		...
		_ = c.repo.MarkFailedRetryable(ctx, record.ID, reason, backoffUntil, expiresAt)
		return nil, execErr
	}
