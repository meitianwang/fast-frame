...
func (e SubscriptionPlanEdges) OrdersOrErr() ([]*PaymentOrder, error) {
	if e.loadedTypes[1] {
		return e.Orders, nil
	}
	...
	for i := range columns {
		...
		case subscriptionplan.FieldOriginalPrice:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				...
				_m.OriginalPrice = new(decimal.Decimal)
				*_m.OriginalPrice = *value.S.(*decimal.Decimal)
			}
...
// QueryOrders queries the "orders" edge of the SubscriptionPlan entity.
func (_m *SubscriptionPlan) QueryOrders() *PaymentOrderQuery {
	return NewSubscriptionPlanClient(_m.config).QueryOrders(_m)
}
