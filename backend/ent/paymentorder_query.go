...
func (_q *PaymentOrderQuery) QueryAuditLogs() *PaymentAuditLogQuery {
	query := (&PaymentAuditLogClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *PaymentOrderQuery) WithAuditLogs(opts ...func(*PaymentAuditLogQuery)) *PaymentOrderQuery {
	query := (&PaymentAuditLogClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PaymentOrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PaymentOrder, error) {
	...
	if query := _q.withAuditLogs; query != nil {
		if err := _q.loadAuditLogs(ctx, query, nodes,
			func(n *PaymentOrder) { n.Edges.AuditLogs = []*PaymentAuditLog{} },
			func(n *PaymentOrder, e *PaymentAuditLog) { n.Edges.AuditLogs = append(n.Edges.AuditLogs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PaymentOrderQuery) loadAuditLogs(ctx context.Context, query *PaymentAuditLogQuery, nodes []*PaymentOrder, init func(*PaymentOrder), assign func(*PaymentOrder, *PaymentAuditLog)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(paymentauditlog.FieldOrderID)
	}
	query.Where(predicate.PaymentAuditLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(paymentorder.AuditLogsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
