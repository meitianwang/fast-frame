...
func (_q *SubscriptionPlanQuery) QueryOrders() *PaymentOrderQuery {
...
func (_q *SubscriptionPlanQuery) WithOrders(opts ...func(*PaymentOrderQuery)) *SubscriptionPlanQuery {
	query := (&PaymentOrderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SubscriptionPlanQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*SubscriptionPlan, error) {
	...
	if query := _q.withOrders; query != nil {
		if err := _q.loadOrders(ctx, query, nodes,
			func(n *SubscriptionPlan) { n.Edges.Orders = []*PaymentOrder{} },
			func(n *SubscriptionPlan, e *PaymentOrder) { n.Edges.Orders = append(n.Edges.Orders, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SubscriptionPlanQuery) loadOrders(ctx context.Context, query *PaymentOrderQuery, nodes []*SubscriptionPlan, init func(*SubscriptionPlan), assign func(*SubscriptionPlan, *PaymentOrder)) error {
	...
	query.Where(predicate.PaymentOrder(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(subscriptionplan.OrdersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
