...
func (_q *PaymentAuditLogQuery) QueryOrder() *PaymentOrderQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(paymentauditlog.Table, paymentauditlog.FieldID, selector),
...
func (_q *PaymentAuditLogQuery) WithOrder(opts ...func(*PaymentOrderQuery)) *PaymentAuditLogQuery {
	query := (&PaymentOrderClient{config: _q.config}).Query()
...
func (_q *PaymentAuditLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PaymentAuditLog, error) {
	...
	if query := _q.withOrder; query != nil {
		if err := _q.loadOrder(ctx, query, nodes, nil,
			func(n *PaymentAuditLog, e *PaymentOrder) { n.Edges.Order = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PaymentAuditLogQuery) loadOrder(ctx context.Context, query *PaymentOrderQuery, nodes []*PaymentAuditLog, init func(*PaymentAuditLog), assign func(*PaymentAuditLog, *PaymentOrder)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*PaymentAuditLog)
	for i := range nodes {
		fk := nodes[i].OrderID
...
func (_q *PaymentAuditLogQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withOrder != nil {
			_spec.Node.AddColumnOnce(paymentauditlog.FieldOrderID)
		}
	}
