...
type PaymentOrder struct {
	...
	RefundRequestedBy *int64 `json:"refund_requested_by,omitempty"`
...
func (*PaymentOrder) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case paymentorder.FieldPayAmount, paymentorder.FieldFeeRate, paymentorder.FieldRefundAmount:
	...
	for i := range columns {
		...
		case paymentorder.FieldPayAmount:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				...
				_m.PayAmount = new(decimal.Decimal)
				*_m.PayAmount = *value.S.(*decimal.Decimal)
			}
		case paymentorder.FieldFeeRate:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field fee_rate", values[i])
			} else if value.Valid {
				_m.FeeRate = new(decimal.Decimal)
				*_m.FeeRate = *value.S.(*decimal.Decimal)
			}
		...
		case paymentorder.FieldRefundAmount:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				...
				_m.RefundAmount = new(decimal.Decimal)
				*_m.RefundAmount = *value.S.(*decimal.Decimal)
			}
		...
		case paymentorder.FieldRefundRequestedBy:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				_m.RefundRequestedBy = new(int64)
				*_m.RefundRequestedBy = value.Int64
			}
...
// QueryAuditLogs queries the "audit_logs" edge of the PaymentOrder entity.
func (_m *PaymentOrder) QueryAuditLogs() *PaymentAuditLogQuery {
	return NewPaymentOrderClient(_m.config).QueryAuditLogs(_m)
}
