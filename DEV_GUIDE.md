...
| 项目 | 说明 |
|------|------|
| **上游仓库** | meitianwang/fast-frame |
| **Fork 仓库** | bayma888/fast-frame-bmai |
...
| 数据库凭据 | user=`fast-frame`, password=`fast-frame`, dbname=`fast-frame` |
...
psql -U fast-frame -h 127.0.0.1 -d fast-frame -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER fast-frame WITH PASSWORD 'fast-frame';
...
### 坑 10：前端测试看似正常，但后端调用失败（配置被批量误改）

**典型现象**：
- 前端按钮点测看起来正常；
- 实际通过 API/客户端调用时返回 `Service temporarily unavailable`；
- 常见于批量修改后突然不可用。

**根因**：
- 批量修改时同时选中不同类型的配置项，导致关键配置被覆盖或丢失。

**修复方案（按优先级）**：
1. **快速修复（推荐）**：在批量修改中补回正确的配置。
2. **彻底重建**：删除并重新添加全部相关配置（最稳但成本高）。

**关键经验**：
- 批量操作前尽量按类型分组，不要混选不同类型的配置项。
...
```bash
# 连接数据库
psql -U fast-frame -h 127.0.0.1 -d fast-frame
...
# 执行 SQL 文件
psql -U fast-frame -h 127.0.0.1 -d fast-frame -f migration.sql
```
...
- [上游仓库](https://github.com/meitianwang/fast-frame)
